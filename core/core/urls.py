...
schema_view = get_schema_view(
    openapi.Info(
        ...
        contact=openapi.Contact(email="contact@snippets.local"),  # Contact email for API support
        license=openapi.License(name="BSD License"),  # API license information
    ),
    public=True,  # Makes the API documentation publicly accessible
    permission_classes=(permissions.AllowAny,),  # Grants access to any user without authentication
)
...
urlpatterns = [
    ...
    path('swagge/output.json/', schema_view.without_ui(cache_timeout=0), name='schema-json'),  # JSON schema output
...
if settings.DEBUG:
    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)  # Handles media files
    urlpatterns += static(settings.STATIC_URL, document_root=settings.STATIC_ROOT)  # Handles static files
