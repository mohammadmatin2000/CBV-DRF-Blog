...
class CustomRegistrationApiView(serializers.ModelSerializer):
    ...
    password1 = serializers.CharField(max_length=255, write_only=True)  # Secondary password field for confirmation
    ...
    class Meta:
        ...
        fields = ('email', 'password', 'password1')  # Specifies the fields included in API response
        extra_kwargs = {
            'password': {'write_only': True},  # Ensures password field is write-only for security
        }
    ...
    def validate(self, data):
        ...
        if data.get('password') != data.get('password1'):
            raise serializers.ValidationError({'detail': 'Passwords must match'})  # Ensures passwords are identical

        try:
            validate_password(data.get('password'))  # Runs Django's built-in password validation checks
    ...
    def create(self, validated_data):
        ...
        """
        validated_data.pop('password1')  # Removes the confirmation field before user creation
        password = validated_data.pop('password')  # Extracts the password
        ...
        if username and password:
            ...
            if not user:  # If authentication fails, return an error
                raise serializers.ValidationError(_('Unable to log in with provided credentials.'),
                                                  code='authorization')
            if not getattr(user, "is_verified", False):  # Ensures user account is verified before allowing login
...
class CustomChangePasswordSerializer(serializers.Serializer):
    ...
    old_password = serializers.CharField(write_only=True)  # Old password field
    new_password = serializers.CharField(write_only=True)  # New password field
    new_password_confirmation = serializers.CharField(write_only=True)  # Confirmation for the new password
    ...
    def validate(self, attrs):
        ...
        if attrs.get('new_password') != attrs.get('new_password_confirmation'):
            raise serializers.ValidationError({'detail': 'Passwords must match'})  # Ensures passwords match

        try:
            validate_password(attrs.get('new_password'))  # Runs password strength validation
        except exceptions.ValidationError as e:
            raise serializers.ValidationError(
                {'new_password': e.messages})  # Raises validation errors if password is weak
...
class CustomProfileUser(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'email', 'first_name', 'last_name', 'image',
                  'designation')  # Specifies the fields included in API response
# ======================================================================================================================
...
class CustomActivationResendSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            raise serializers.ValidationError({'detail': _('User does not exist.')})  # Ensures user exists

        attrs['user'] = user_obj  # Stores user data in attributes

        if user_obj.is_verified:
            raise serializers.ValidationError(
                {'detail': 'User account is already verified.'})  # Prevents resending activation to verified users
