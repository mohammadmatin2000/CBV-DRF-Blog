...
from .views import (
    ...
    CustomProfileUserView,
    CustomActivationView,
    CustomActivationResendView,
...
urlpatterns = [
    ...
    path(
        "activation/resend/",
        CustomActivationResendView.as_view(),
        name="resend",
    ),
    ...
    path(
        "token/login/", CustomAuthToken.as_view(), name="token-login"
    ),
    # - Authenticates users and provides authentication tokens.
    path(
        "token/logout/",
        CustomDeleteToken.as_view(),
        name="token-logout",
    ),
