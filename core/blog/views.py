...
class IndexView(TemplateView):
    template_name = 'index.html'  # Specifies which template should be used

    def get_context_data(self, **kwargs):
        """
        This method adds extra context data to the template.
        """
        context = super().get_context_data(**kwargs)  # Gets the default context
        context['title'] = 'Blog'  # Adds a title to the context
        context['posts'] = Post.objects.all()  # Fetches all blog posts
        return context
...
class PostFormView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        """
        form.instance.author = self.request.user  # Sets the author to the currently logged-in user
        return super().form_valid(form)  # Calls the parent method to complete form submission
